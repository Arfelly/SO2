...
def procesar_carpeta_serial(ruta_base, nombre_serial, ruta_salida):
    ...
    if datos_totales:
        ...
        with open(ruta_archivo_salida, 'w') as file_salida:
            file_salida.write("tiempo\tflujoKgs\tflujoTond\tmaxfKgs\tmaxfTond\n")

            for linea in datos_totales:
                Fecha_H = f"{linea[0]} {linea[1]}"
                tiempo = datetime.strptime(Fecha_H, '%Y-%m-%d %H:%M:%S')
                flujoKgs = linea[2]
                flujoTond = '{:.2f}'.format(float(flujoKgs) * 86.4)

                max_fKgs = max([float(x[2]) for x in datos_totales if x[0] == linea[0]])
                maxfTond = '{:.2f}'.format(float(max_fKgs) * 86.4)

                file_salida.write(f"{tiempo}\t{flujoKgs}\t{flujoTond}\t{max_fKgs}\t{maxfTond}\n")                
...
def graficar_desde_txt(archivo, ruta_archivo_salida):
    ...
    if tiempo:  # Verificar si se cargaron fechas correctamente
        ...
        axs[0].plot(tiempo[::31], maximos_diarios_Kgs, 'k*', alpha=0.3, label='Máx. diario Kg/s', markersize=1)
        axs[1].plot(tiempo[::31], maximos_diarios_Tond, 'k*', alpha=0.3, label='Máx. diario Ton/días', markersize=1)
        ...
        for i, fecha in enumerate(tiempo[::31]):
            ...
            axs[1].annotate(f'{maximos_diarios_Tond[i]:.1f}',
        ...
        axs[0].set_ylabel('Flujo Kg/s')
        axs[1].set_ylabel('Flujo Ton/Días')
